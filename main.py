...
# ग्लोबल वेरिएबल डायनेमिक बटन वॉटरमार्क स्टोर करने के लिए
...
photologo = 'https://files.catbox.moe/28epkq.jpg'
photoyt = 'https://files.catbox.moe/28epkq.jpg'
photocp = 'https://files.catbox.moe/28epkq.jpg'
photozip = 'https://files.catbox.moe/28epkq.jpg'
...
image_urls = [
    "https://files.catbox.moe/28epkq.jpg",
    "https://files.catbox.moe/28epkq.jpg",
]

# --- 7 रंगों वाला और सेंटर वॉटरमार्क लॉजिक (FFMPEG) ---
def apply_screen_watermark(video_file):
    text_wm = DYNAMIC_WATERMARK
    if not text_wm:
        return video_file  # अगर बटन से सेट नहीं है, तो बिना वॉटरमार्क के भेजेगा
        
    temp_output = "wm_" + video_file
    
    # 7 बेहतरीन रंगों की लिस्ट
    colors_list = ["yellow", "white", "cyan", "green", "magenta", "orange", "lightcircle"]
    chosen_color = random.choice(colors_list)
    if chosen_color == "lightcircle":
        chosen_color = "0xFF6666" # हल्का लाल रंग
        
    # FFmpeg कमांड - वीडियो के बिल्कुल बीचों बीच सेट करने के लिए
    cmd = f'ffmpeg -i "{video_file}" -vf "drawtext=text=\'{text_wm}\':x=(w-text_w)/2:y=(h-text_h)/2:fontsize=32:fontcolor={chosen_color}" -c:a copy "{temp_output}" -y'
    subprocess.run(cmd, shell=True)
    
    if os.path.exists(temp_output):
        os.remove(video_file)
        os.rename(temp_output, video_file)
        print(f"[INFO] Center Watermark '{text_wm}' with color '{chosen_color}' applied successfully!")
    return video_file
...
async def restart_handler(_, m):
    if m.chat.id != OWNER:
        ...
        await m.reply_text("𝐁𝐨𝐭 𝐢𝐬 𝐑e𝐬𝐞𝐭𝐢𝐧𝐠...", True)
...
async def help_button(client, callback_query):
  ...
  caption = (
        ...
        f"➥ /getcookies – Download Bot Cookies\n"
...
async def watermark_button(client, callback_query):
  global DYNAMIC_WATERMARK
  chat_id = callback_query.message.chat.id
  
  editable = await callback_query.message.reply_text("✍️ **कृपया वीडियो स्क्रीन के लिए अपना नया वाटरमार्क टेक्स्ट भेजें:**\n*(E.g. सचिन शर्मा)*")
  
  try:
      user_input: Message = await client.listen(chat_id, timeout=30)
      if user_input.text:
          DYNAMIC_WATERMARK = user_input.text.strip()
          await user_input.delete(True)
          await editable.edit(f"✅ **सफलतापूर्वक सेट!**\nअब वीडियो स्क्रीन के बीच में **'{DYNAMIC_WATERMARK}'** वाटरमार्क 7 अलग-अलग रंगों में चेंज होकर आएगा।")
      else:
          await editable.edit("⚠️ **गलत इनपुट! केवल टेक्स्ट संदेश ही भेजें।**")
  except asyncio.TimeoutError:
      await editable.edit("⏳ **समय समाप्त!** वाटरमार्क सेट करने के लिए दोबारा क्लिक करें।")
...
async def txt_handler(bot: Client, m: Message):  
    ...
    await editable.edit("**Enter 𝐏𝐖/𝐂𝐖/𝐂𝐏 Working Token For 𝐌𝐏𝐃 𝐔𝐑𝐋 or send /d**\n\n<blockquote><b>Note: If you are downloading Classplus Video, Make sure you joined @sachinbotverification this channel.</b></blockquote>")
    try:
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            # --- SAFE LOOP PROTECTION WITH ERROR HANDLING ---
            try:
                ...
                # --- FIX YOUTUBE LOGIC FOR BATCH FILTERS ---
                if "youtube.com" in Vxy or "youtu.be" in Vxy:
                    cmd = f'yt-dlp --cookies youtube_cookies.txt -f "bv*[height<={raw_text2}][ext=mp4]+ba[ext=m4a]/b[height<={raw_text2}]" "{url}" -o "{name}.mp4"'
                    ...
                    filename = apply_screen_watermark(filename)
                ...
                if "acecwply" in url:
                    ...
                    url = f"https://cpapi-rjbs-1l0p.onrender.com/extract_keys?url={url}@sachinbotverification&user_id={user_id}"
                    mpd, keys = helper.get_mps_and_keys(url)
                    url = mpd
                    keys_string = " ".join([f"--key {key}" for key in keys])

                elif "classplusapp" in url:
                    signed_api = f"https://cpapi-rjbs-1l0p.onrender.com/extract_keys?url={url}@sachinbotverification&user_id={user_id}"
                ...
                try:
                    ...
                    if "drive" in url:
                        ...
                        Show1 = f"<blockquote>🚀𝐏𝐫𝐨𝐠𝐫𝐞𝐬𝐬 » {progress:.2f}%</blockquote>\n┃\n" \
                               ...
                               f"━━━━━━━━━━━━━━━━━━━━━━━━\n" \
                               f"<blockquote><b>⚡D0wDownloadiɴɢ Eɴᴄʀʏᴘᴛᴇᴅ Sᴛᴀʀᴛᴇᴅ...⏳</b></blockquote>\n┃\n" \
                               f'┣💃𝐂𝐫𝐞𝐝𝐢𝐭 » {CR}\n┃\n' \
                               f"╰━📚𝐁𝐚𝐭𝐜𝐡 » {b_name}\n" \
                               f"━━━━━━━━━━━━━━━━━━━━━━━━━\n" \
                        ...
                        Show1 = f"<blockquote>🚀𝐏𝐫𝐨𝐠𝐫𝐞𝐬𝐬 » {progress:.2f}%</blockquote>\n┃\n" \
                               ...
                               f"╰━🖇️𝐑𝐞𝐦𝐚𝐢𝐧 » {remaining_links}\n" \
                               f"━━━━━━━━━━━━━━━━━━━━━━━━\n" \
                               f"<blockquote><b>⚡Dᴏᴡɴʟ2ᴏᴀᴅɪɴɢ Sᴛᴀʀᴛᴇᴅ...⏳</b></blockquote>\n┃\n" \
                               f'┣💃𝐂𝐫𝐞𝐝𝐢𝐭 » {CR}\n┃\n' \
                               f"╰━📚𝐁𝐚𝐭𝐜𝐡 » {b_name}\n" \
                               f"━━━━━━━━━━━━━━━━━━━━━━━━━\n" \
                               f"<blockquote>📚𝐓𝐢𝐭𝐥𝐞 » {name}</blockquote>\n┃\n" \
                    ...
                    # --- 404 AND ERROR HANDLER SKIP LOGIC ---
                    await bot.send_message(channel_id, f'⚠️ **Downloading Skipped** ⚠️\n**Index:** {count}\n**Name:** `{name1}`\n<blockquote><b>Reason: File Not Found or Link Expired (404)</b></blockquote>')
                    count += 1
                    failed_count += 1
                    continue
...
async def text_handler(bot: Client, m: Message):
    ...
    # --- FIX SINGLE LINK YOUTUBE EXTRACTION ---
    if "youtube.com" in link or "youtu.be" in link:
        await editable.delete()
        name = "YouTube_Video_" + str(int(time.time()))
        cmd = f'yt-dlp --cookies youtube_cookies.txt -f "bv*[ext=mp4]+ba[ext=m4a]/b" "{link}" -o "{name}.mp4"'
        prog = await m.reply_text("📥 **Downloading Single YouTube Video...**")
        os.system(cmd)
        await prog.delete()
        
        filename = apply_screen_watermark(f"{name}.mp4")
        
        cc = f'🎞️ **YouTube Video**\n🌟 **Extracted By:** {CREDIT}'
        await helper.send_vid(bot, m, cc, filename, "/d", name, None, m.chat.id)
        return
    ...
    try:
            ...
            if "visionias" in url:
                async with ClientSession() as session:
                    async with session.get(url, headers={'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9', 'Accept-Language': 'en-US,en;q=0.9', 'Cache-Control': 'no-cache', 'Connection': 'keep-alive', 'Pragma': 'no-cache', 'Referer': 'http://www.visionias.in/', 'Sec-Fetch-Dest': 'iframe', 'Sec-Fetch-Mode': 'navigate', 'Sec-Fetch-Site': 'cross-site', 'Upgrade-Insecure-Requests': '1', 'User-Agent': 'Mozilla/5.0 (Linux; Android 12; RMX2121) AppleWebKit/537.36 (KHTML, Gecko) Chrome/107.0.0.0 Mobile Safari/537.36', 'sec-ch-ua': '"Chromium";v="107", "Not=A?Brand";v="24"', 'sec-ch-ua-mobile': '?1', 'sec-ch-ua-platform': '"Android"',}) as resp:
                        text = await resp.text()
                        url = re.search(r"(https://.*?playlist.m3u8.*?)\"", text).group(1)
...
def reset_and_set_commands():
    ...
    commands = [
        ...
        {"command": "getcookies", "description": "📂 Download Active Cookies"},
